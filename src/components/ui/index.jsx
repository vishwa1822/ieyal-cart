...
export function Button({ className, variant = "default", size = "default", children, ...props }) {
  const variants = {
    ...
    outline: "border border-border bg-surface text-[var(--color-text)] hover:bg-[var(--color-bg)]",
  ...
  return (
    <button
      className={cn(
        "inline-flex items-center justify-center rounded-btn font-medium text-sm transition-all active:scale-[0.98] disabled:opacity-50 disabled:pointer-events-none",
        variants[variant],
        sizes[size],
        className
      )}
...
export function Card({ className, children, ...props }) {
  return (
    <div className={cn("rounded-card border border-border bg-surface shadow-premium", className)} {...props}>
      {children}
    </div>
  );
}
...
export function CardTitle({ className, children, ...props }) {
  return <h3 className={cn("text-lg font-semibold leading-tight", className)} {...props}>{children}</h3>;
}
...
export const Input = forwardRef(function Input({ className, ...props }, ref) {
  return (
    <input
      ref={ref}
      className={cn(
        "flex h-10 w-full rounded-btn border border-border bg-surface px-3 text-sm outline-none transition-colors focus:border-primary focus:ring-2 focus:ring-primary/20",
        className
      )}
      {...props}
    />
...
export function Badge({ className, variant = "default", children, ...props }) {
  const variants = {
    ...
    secondary: "bg-[var(--color-bg)] text-muted",
